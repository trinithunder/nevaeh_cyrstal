module Nevaeh
  module Generators
    class DefaultInstallGenerator < Rails::Generators::NamedBase
  ...
  models.each {|model| create_scaffold(model)}
      ...
      def create_scaffold(model)
        ...
        when 'post'
          generate "scaffold", "#{model_name} title:string content:text"
        when 'comment'
          generate "scaffold", "#{model_name} body:text post_id:integer user_id:integer"
        when 'user'
          generate "scaffold", "#{model_name} first_name:string last_name:string email:string password:string"
        # Add more cases as needed
        else
          puts "No scaffold defined for #{model_name}"
        end
